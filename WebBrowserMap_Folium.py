# -*- coding: utf-8 -*-
"""
Created on Wed May 31 11:38:27 2023
...
connection_string = "mysql+mysqlconnector://ssmsuser:ssmsuser@EU012VM0589.tp.tpnet.intra:3306/ssms082755c"
sql_engine = sql.create_engine(connection_string, echo=True)
query  = "select Area, Structure, dp_mp, Name, RD, am_serial_no, Weight, DESCRIP, Vendor, fab_start_date, fab_completed_date, srn_date from ssms_rddump"
...
########################################### START: CREATE MARKERS AND ADD TO THE MAP #########################################################
    ...
    #create a line between marker draggable and circle marker - NON FUNZIONA ANCORA
    # vedi esempio a https://stackoverflow.com/questions/74253482/adding-a-tooltip-on-a-line-between-two-markers-in-folium-using-python
    """
    w = {
    ...
    latlong_origin = [row.Lat, row.Long]
    latlong_destin = [row.Lat, row.Long]
    folium.PolyLine([[latlong_origin[0], latlong_origin[1]], [latlong_destin[0], latlong_destin[1]]],w, fill_color='blue').add_to(m)
  """
...
"""
#create an html file and open in the webbrowser
mapName = "RDCG"
map_html = mapName + '.html'
m.save(mapName + '.html') # save the map as an html object to open in the browser

webbrowser.open(map_html)  # open file in webbrowser
"""